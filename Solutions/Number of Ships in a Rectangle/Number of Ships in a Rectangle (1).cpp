...
class Solution {
public:
    // divide and conquer along x direction (rectangle is a line)
    int countInRow(int x1, int x2, int y, Sea &sea) {
        // discard entire line
        if (!sea.hasShips(vector<int>({x2, y}), vector<int>({x1, y})))
            return 0;
        ...
        // split grid in half along x-axis
        int mid = x1 + (x2 - x1) / 2;
        return countInRow(x1, mid, y, sea) + countInRow(mid + 1, x2, y, sea);
    }
    
    // divide and conquer along y direction (rectangle is a grid)
    int countInGrid(int x1, int y1, int x2, int y2, Sea &sea) {        
        // discard entire grid
        if (!sea.hasShips(vector<int>({x2, y2}), vector<int>({x1, y1})))
            return 0;
        
        // divide and conquer for points in single row (rectangle is a line)
        if (y1 == y2)
            return countInRow(x1, x2, y1, sea);
        
        // split grid in half along y-axis
        int mid = y1 + (y2 - y1) / 2;
        return countInGrid(x1, y1, x2, mid, sea) + countInGrid(x1, mid + 1, x2, y2, sea);
    }
    ...
    int countShips(Sea sea, vector<int> topRight, vector<int> bottomLeft) {
        ...
        return countInGrid(x1, y1, x2, y2, sea);
    }
};
